...
def crawl_rottentomatoes(soup):
    ...
    critic_vote = div_score_element.find("rt-button", {"slot": "criticsScore"}).text.strip()
    meta_score = div_score_element.find("rt-link", {"slot": "criticsReviews"}).text.strip()

    print(critic_vote)
    print(string_to_num(meta_score))
